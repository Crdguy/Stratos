...
class EHlookup(commands.Cog):
    ...
    async def module(self, ctx):
        ...
        def getComponent(file):
            with open("modulelookuptable.csv") as lookuptableraw:
                lookuptable = csv.reader(lookuptableraw, delimiter=",")
                
                for row in lookuptable:
                    if module == row[0]:
                        file = "Database/Component/"+row[1]
                        break
                with open(file) as jsonfile:
                    ...
                    #get faction
                    try:
                        with open("factionlookuptable.csv") as lookuptableraw:
                            lookuptable = csv.reader(lookuptableraw, delimiter=",")
                            for row in lookuptable:
                    ...
                    allMods = []
                    #print("getting mods")
                    with open("modificationlookuptable.csv") as lookuptableraw:            
                        lookuptable = csv.reader(lookuptableraw, delimiter=",")
                        for row in lookuptable:
                            x = 0
        ...
        def getAmmunition(ammoID):
            ...
            if config[";ehlookup Settings"]["generateWorkshopLevels"] == "True":
                ...
                for f in glob("Database/Ammunition/*.json"):
                ...
                try:
                    if data["DamageType"] == 0:
                        damageType = "Kinetic"
                    elif data["DamageType"] == 1:
                        damageType = "Energy"
                    elif data["DamageType"] == 2:
                        damageType = "Heat"
                    elif data["DamageType"] == 3:
                        damageType = "Direct"
                    else:
        ...
        def getWeapon(weaponID):
            ...
            if config[";ehlookup Settings"]["generateWorkshopLevels"] == "True":
                ...
                for f in glob("Database/Weapon/*.json"):
                    with open(str(f)) as file:
                        data = json.load(file)
                        x = 0
                        if data["Id"] == weaponID:
        ...
        def getStats(statsID,platformSize):
            ...
            if config[";ehlookup Settings"]["generateWorkshopLevels"] == "True":
                ...
                for f in glob("Database/Component/Stats/*.json"):
                    with open(str(f)) as file:
                        data = json.load(file)
                        x = 0
                        if data["Id"] == statsID:
    ...
    async def ship(self, ctx):
        ...
        def getdata(file):
            ...
            return sclass,satclass,hp,baseweight,minweight,cost,stars,kineticresistance,thermalresistance,energyresistance,regen
        ...
        sclass,satclass,hp,baseweight,minweight,cost,stars,kineticresistance,thermalresistance,energyresistance,living = getdata(ship)
        if sclass == "None":
        ...
        if nolayout == False:
            ...
            endTime = int(round(time.time() * 1000))
            emb.description = "Response time: {}ms\nImage generation time: {}ms".format(endTime-startTime,L2Presponsetime)
            await ctx.send(files=[f,f2],embed=emb)
        else:
            endTime = int(round(time.time() * 1000))
            emb.description = "Response time: {}ms".format(endTime-startTime)
            await ctx.send(embed=emb,files = [f,f2])



    '''
    @ship.error
    async def ship_error(ctx,err):
